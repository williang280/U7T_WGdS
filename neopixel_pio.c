 ...
 */
void init_joystick() {
    // Inicializa o ADC para os eixos X e Y
    adc_init();
    adc_gpio_init(26);
    adc_gpio_init(27);
    ...
    gpio_pull_up(JOYSTICK_BTN_PIN);  // Habilita pull-up interno
}
...
bool is_joystick_pressed() {
    return !gpio_get(JOYSTICK_BTN_PIN);  // Retorna true quando o botão é pressionado (nível baixo)
}
...
/**
 * Toca um som de conclusão no buzzer
 */
void play_success_sound() {
    ...
    for(int i = 0; i < 3; i++) {
        pwm_set_gpio_level(BUZZER_A_PIN, 15625);  // Frequência baixa
        pwm_set_gpio_level(BUZZER_B_PIN, 15625);
        sleep_ms(100);
        
        pwm_set_gpio_level(BUZZER_A_PIN, 31250);  // Frequência média
        pwm_set_gpio_level(BUZZER_B_PIN, 31250);
        sleep_ms(100);
        
        pwm_set_gpio_level(BUZZER_A_PIN, 46875);  // Frequência alta
        pwm_set_gpio_level(BUZZER_B_PIN, 46875);
        sleep_ms(150);
    }
...
int main() {
  ...
  // Contador de pressionamentos do joystick
  int joystick_press_count = 0;
  ...
  while (true) {
    ...
    if (current_joystick_state && !last_joystick_state) {
      ...
      if (joystick_press_count == 1) {
        // Primeiro pressionamento: LEDs brancos aleatórios
        ...
        npSetLED(led1, 255, 255, 255);
        npSetLED(led2, 255, 255, 255);
        npWrite();
        
        display_message("FALHAS LOCALIZADAS");
      } 
      else if (joystick_press_count == 2) {
        // Segundo pressionamento: LEDs azuis
        for (int i = 0; i < LED_COUNT; i++) {
          npSetLED(i, 0, 0, 255);  // Azul
        }
        npWrite();
        display_message("ENVIANDO DADOS");
        play_success_sound();
        
        // Aguarda um momento com a cor azul
        sleep_ms(2000);
        ...
        npWrite();
        display_message("POSTES DETECTADOS");
